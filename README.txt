...
Операционная система работает с протоколами TCP/IP(и с несколькими протоколами прикладного уровня), файловой системой ext2, пользовательскими приложениями в  мультипоточном режиме(процессы тоже создает). Тестировалась, отлаживалась и запускалась только в Bochs. "Простая" и "учебная" в первую очередь из-за старого железа(драйверов для него), менеджеров выделения памяти и потоков(просто выбырает "следующий" из списка). "Спящие" потоки из списка убираются), впринципе, если их прокачать, то будет очень неплохо, хотя и так всё работает. Загружается с дискеты 1,44 Mb.
